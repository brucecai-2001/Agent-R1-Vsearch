...
def compute_score_format(solution_str):
    ...
    try:
        ...
        if assistant_blocks:  # 确保有至少一个assistant块
            ...
            if last_assistant_block.count('<think>') == 1 and last_assistant_block.count('</think>') == 1 and last_assistant_block.count('<answer>') == 1 and last_assistant_block.count('</answer>') == 1:
